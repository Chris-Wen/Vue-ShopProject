...
class WebShopAction extends Action {
	...
	public function getAllOrder() {      //已付款或交易成功   获取 二维关联数组
		...
		$allow_origin = array(    
			'http://shop.73776.com',    	
			'http://192.168.1.162:8085'   	//本机本地直接请求
		);    
		...
		class WebShopAction extends Action {
			public function test() {
				// $name = $_POST['name']or die('{"code":401,"msg":"type required"}');
				echo cookie('browser')." ---that cookie";
				foreach (getallheaders() as $name => $value) {
					echo "$name: $value\n";
				}
			...
			public function getIndexInfo() {	//首页信息展示
				// cookie('test', '测试cookie存储过程');
				$list['banner'] = M()->query("SELECT logo,link FROM rc_shop_goods WHERE status=1 AND sort=1 ORDER BY sort1"); 
		
				$list['hotSales'] = M()->query("SELECT id, sname, logo, price, score, realprice, detail, title, freight_type, pic FROM rc_shop_goods WHERE status=1 AND sort=2 ORDER BY sort1");
		
				$list['session_uid'] = session('uid');
				echo json_encode($list);
			}
			...
			public function vaileLogin(){				//验证cookie 免密登录 		
				...
				@$utoken = $_POST['utoken'];		//浏览器支持cookie且未失效情况下，不传参
				if ( !$ucookie && !$utoken ) { die('{"code":401,"msg":"did not login"}'); }
				$m = M('shop_user_cookie');
		
				//新cookie  用于免密登录
				$cookie = hash_hmac( "sha256", session('PHPSESSID').time(), md5("zdkjadmin"));
				if ($ucookie) {				//ucookie 验证登录
					$cookie_res = $m->where("cookie='".$ucookie."'and cook_times<1000")->find();
					if ($cookie_res){
						$uid = $cookie_res['uid'];
		
						//更新cookie, 	cookie登录记录
						$times = M('shop_user_cookie')->where("uid=".$uid)->getField('cook_times');				
						$data_cookie['cookie'] = $cookie;
						$data_cookie['cook_times'] = $times + 1;
		
						$insert_cookie = M('shop_user_cookie')->where("uid=".$uid)->save($data_cookie);				
					} else {				//ucookie 失效，需要登录
						echo('{"code":401, "msg":"login over frequency"}');
						return;
					}
				} else {					//浏览器不支持cookie情况下
					$inner_time = date('Y-m-d H:i:s',strtotime('-7day'));
					$token_res = M('shop_user_token')->where(" token = $utoken AND logintime > $inner_time")->find();
					if( !$token_res ) {		//token失效，需要登录
						echo ('{"code":401, "msg":"need login"}');
						return;
					} 
					$uid = $token_res['uid'];
					//更新cookie
					$times = M('shop_user_cookie')->where("uid=".$uid)->getField('times');				
					$data_cookie['cookie'] = $cookie;
					$data_cookie['cook_times'] = 0;
					$data_cookie['times'] = $times + 1;
